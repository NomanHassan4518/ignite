...
import { decreseToQuantity } from '../Services/Action/Action'
import { increaseToQuantity } from '../Services/Action/Action'
import { useNavigate } from 'react-router-dom'
import { useDispatch , useSelector } from 'react-redux'



const MobileShopingCart = () => {
    ...
    <>
      ...
      <div>
        ...
        <button type="button" className="relative inline-flex items-center p-3 text-sm font-medium text-center text-black  focus:outline-none focus:ring-0 dark:bg-none  dark:focus:ring-0" onClick={drawweOpen}>
          ...
          <div className="absolute inline-flex items-center justify-center w-6 h-6 text-xs font-bold text-white bg-black border-2 border-white rounded-full -top-[0.1rem] -right-[0.2rem] dark:border-gray-900" >{
            numberOfItems}</div>
        </button>
      ...
      >
        <div >
          <div className={myData.length !== 0 ? "hidden" : "block"}>
          ...
          <div className={myData.length !== 0 ? "block h-[30rem]  overflow-y-scroll" : "hidden"}>

            {
              cardProduct?.map((items, index) => (
                <div className='flex  justify-between items-center mt-12'  key={index}>
                  <div className='group cursor-pointer relative'>
                    <img src={items.Product?.image.original} alt="" className='w-[160px] h-28' />
                  ...
                  <div className='ml-2'>
                    ...
                    <p className='mt-1 text-gray-400'>Unit Price: {price}</p>
          ...
          <button className='flex w-full  flex-col pt-2 pb-2' onClick={CheckOut} >
                        <div className={`w-full bg-gray-400 hover:bg-[#000] text-white hover:text-white rounded-md cursor-not-allowed px-2 py-5 ${cardProduct?.length===0 ? 'cursor-not-allowed':'cursor-pointer'} flex items-center justify-center `} onClick={close}>
                            <span className='w-ful text-lg pe-5 md:text-[20px] font-semibold'>Proceed to Checkout</span>
                            <span className='mx-auto text-lg md:text-[20px] font-semibold '>| {cardProduct?.length !== 0 ?  `SAR ${price}` : "SAR 0.00"}</span>
                        </div>
                    </button>
        </div>
      </Drawer>
...
export default MobileShopingCart
