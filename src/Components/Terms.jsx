import React from 'react'

const Terms = () => {
  return (
    <div>
            <section class="bg-[#00000073] dark:bg-gray-900 h-[15rem] lg:h-[20rem] flex justify-center items-center">
                <div class="py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16">
                    <p class="mb-2 satisfy text-lg font-normal text-white lg:text-xl sm:px-16 lg:px-48 dark:text-gray-400">explore</p>
                    <h1 class="mb-4 text-xl  lg:text-3xl font-bold tracking-tight leading-none text-white  dark:text-white">Terms of Service</h1>
                </div>
            </section>
            <div className="mt-12 lg:mt-14 xl:mt-16 mb-10 border-b border-gray-300"></div>
        </div>
  )
}

export default Terms
